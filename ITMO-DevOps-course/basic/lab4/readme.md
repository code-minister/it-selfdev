...
Проверяющие могут начать [отсюда](#шаг-31-создание-bed-ci-cd)
...
После 11 неудачных запусков плохой ci-cd наконец заработал и можно проверить его на деле.  
Итак, вот наше приложение в проде:
![alt text](images/image-2.png)
И мы решили, что нужно изменить "Жирафы" на "Жирафики". Вносим изменения и делаем git push. И, ураа, оно заработало:
...
И в этом моменте у меня просто перестали пулиться образы, а DockerHub открываться без ***. Хотя вот буквально только что всё работало. При чем собранные образы туда пушаться. То есть переменные из Hashicorp Vault берутся. Так я и не понял заблокирован DockerHub сейчас в РФ или нет.


# Вместо вывода
То что я хотел реализовать я реализовал, пока всё не сломалось. И хотя этот пайплайн имеет много проблем, уже может использоваться с некоторыми доработками.
