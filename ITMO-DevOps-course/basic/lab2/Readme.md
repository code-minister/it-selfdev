...
При создании данного манифеста я решил объединить всё, что касается БД, чтобы не дробить логику слишком сильно. Первым идет StatefulSet, если не ошибаюсь, он полезен при создании кластеров СУБД, а также повышает надежность данных. Здесь же создаются шаблон PVC, который находит или же создают новый PV для конкретной реплики БД. Потом создаётся Headless Service, который не имеет встроенной балансировки и позволяет обращаться к каждому поду сета отдельно. Это важно если у нас есть например разделение на ReadWrite, ReadOnly etc. P.S. Впоследствии пришлось сделать явный маппинг переменных окружения
```yaml
apiVersion: apps/v1
kind: StatefulSet
metadata:
...
Тут мы наконец создаём Deployment для нашего бекенда и сервис к нему, подгружаем все переменные окружения.
```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
Тут знакомимся с CronJob, у нас будет запускать под, который каждые 5 минут будет создавать отчет.
```yaml
apiVersion: v1
...
Согласно Best Practices хелма каждое определение ресурса должно быть в собственном шаблоне, а также содержать название этого ресурса в названии написанном dashed notation (через тире). Поэтому нужно будет разделить манифесты, которые мы ранее объединили для удобства. Кроме того, везде следует удалить определение `namespace`

### Шаг 2: Создаем values.yaml
Записываем в файле все атрибуты, которые мы хотим динамически изменять:
```yaml
backend:
  replicaCount: 2
  image:
...
Проверяем всё командой `helm template my-test-release ./voting-app-chart`
...
# Вместо вывода
По сравнению с докером тут пришлось обработать гораздо больший объём информации, чтобы добросовестно ознакомиться даже с базовыми ресурсами и практиками, но теперь и это кажется довольно просто. Зато WSL ни разу не сломалась ))
